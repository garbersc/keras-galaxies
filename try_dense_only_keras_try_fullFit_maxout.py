...
TRAIN_LOSS_SF_PATH = "trainingNmbrs_keras_ev_adam_lessReduction_3moreDense_noOrthInit.txt"

#TARGET_PATH = "predictions/final/try_convnet.csv"
WEIGHTS_PATH = "analysis/final/try_convent_keras_ev_adam_lessReduction_3moreDense_noOrthInit.h5"
...
if debug :print model2.output_shape
...
#newShape=lambda x : (model.output_shape[0]/8, 48600)#x[0]*x[1]*x[2]*x[3]//BATCH_SIZE)

#print newShape(model.output_shape)

#merge input variations
model.add( Lambda( lambda x: x.reshape((x.shape[0]/4/N_INPUT_VARIATION, x.shape[1]*4*N_INPUT_VARIATION)), output_shape = lambda x : (x[0]/4/N_INPUT_VARIATION, x[1]*4*N_INPUT_VARIATION) )  )
...
model.add(Dense(output_dim=2048 , activation='tanh', weights = dense_weight_init_values(model.output_shape[-1],2048,orth_style=False) )) #TODO  different output_dim?

model.add(Dense(output_dim=2048 , activation='tanh', weights = dense_weight_init_values(model.output_shape[-1],2048,orth_style=False) ))
...
if not USE_ADAM:
	...
	model_norm.compile(loss='mean_squared_error', optimizer=adam )#, metrics=[rmse, 'categorical_accuracy',kaggle_sliced_accuracy])
	model_noNorm.compile(loss='mean_squared_error', optimizer=adam)

	model_norm_metrics.compile(loss='mean_squared_error', optimizer=adam, metrics=[rmse, 'categorical_accuracy',kaggle_sliced_accuracy])
...
try:
  for i in range(EPOCHS/VALIDATE_EVERY if not EPOCHS%VALIDATE_EVERY else EPOCHS/VALIDATE_EVERY+1 ):
	...
	if len(timedeltas)>10: timedeltas=timedeltas[-10:]
