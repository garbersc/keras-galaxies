...
TRAIN_LOSS_SF_PATH = "trainingNmbrs_keras_maxout_start_w_winsolWeights_noDropout.txt"

ANALYSIS_PATH = "analysis/final/try_convent_keras_maxout_start_w_winsolWeights_noDropout.pkl"
WEIGHTS_PATH = "analysis/final/try_convent_keras_maxout_start_w_winsolWeights_noDropout.h5"
...
model.add(Merge([model1, model2], mode=kaggle_input , output_shape=lambda x: (BATCH_SIZE*4*N_INPUT_VARIATION, NUM_INPUT_FEATURES, PART_SIZE, PART_SIZE) , arguments={'part_size':PART_SIZE, 'n_input_var': N_INPUT_VARIATION, 'include_flip':True, 'random_flip':False}  ))
...
if getWinSolWeights:
	...
	for l in model_norm.layers:
		...
		if len(l_weights)==len(w_kSorted):
			...
			for i in maxdense_layers_ids:
				if debug: print('--- ',i, np.shape(w_kSorted[2*i]))
				w_kSorted[2*i] = w_kSorted[2*i].reshape( ( 2 , np.shape(w_kSorted[2*i])[0] , np.shape(w_kSorted[2*i])[1]/2 ) )		
				w_kSorted[2*i+1] = w_kSorted[2*i+1].reshape( ( 2 , np.shape(w_kSorted[2*i+1])[0]/2 ) )	
