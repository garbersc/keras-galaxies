...
CHUNK_SIZE = 3200 # 10000 # this should be a multiple of the batch size
...
l5 = layers.DenseLayer(l4bc, n_outputs=37, weights_std=0.01, init_bias_value=0.1, dropout=0.5, nonlinearity=layers.identity)
...
if DO_PRINT_FILTERS:
    print
    print "print filter"
    if not os.path.isdir("filterImages"): os.mkdir("filterImages")  
    start_time = time.time()
...
#filter of layer 1 , output format should be  (32, 45, 45, 128)
    ...
    with open("filterImages/whitePrediction.txt", 'w')as f:
	f.write(" %s " % (blackPrediction))

    l1_output = theano.function([idx], l1a.output(), givens=givens)

    filters1 = l1_output(0).transpose(3,0,1,2)[0]
    if not os.path.isdir("filterImages/l1white"): os.mkdir("filterImages/l1white")  
    for i in range(0,32):
     # if i==0:
     ...
     #   with open("imageTest.txt", 'w')as f: 
      ...
      plt.imshow(filters1[i],interpolation='none') #Needs to be in row,col order
      plt.savefig("filterImages/l1white/%s.jpg" % i)


    filters2 = l1_output(0).transpose(3,0,1,2)[1]
    if not os.path.isdir("filterImages/l1white2"): os.mkdir("filterImages/l1white2")  
    for i in range(0,32):
      plt.imshow(filters2[i],interpolation='none') #Needs to be in row,col order
      plt.savefig("filterImages/l1white2/%s.jpg" % i)

    filters3 = l3_output(0).transpose(3,0,1,2)[0]
    if not os.path.isdir("filterImages/l3white"): os.mkdir("filterImages/l3white")  
    for i in range(0,64):
      plt.imshow(filters3[i],interpolation='none') #Needs to be in row,col order
      plt.savefig("filterImages/l3white/%s.jpg" % i)
