...
class OptimisedDivGalaxyOutputLayer(object):
    ...
    def answer_probabilities(self, *args, **kwargs):
        ...
        normalisation_denoms = T.dot(input_clipped, self.normalisation_mask) + 1e-7 # small constant to prevent division by 0
	...
	if self.categorised:
		...
		for k in xrange(0,self.mb_size):
			...
			if i==1: z = z * T.gt(input_q[0][1],0.6)
			if i==2: z = z * T.gt(input_q[1][1],0.6)
			if i==3: z = z * T.gt(input_q[1][1],0.6)
			if i==4: z = z * T.gt(input_q[1][1],0.6)
			if i==6: z = z * T.gt(input_q[0][0],0.6)
			if i==9: z = z * T.gt(input_q[4][0],0.6)
			if i==10: z = z * T.gt(input_q[4][0],0.6)
