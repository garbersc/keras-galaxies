...
def constant(shape, scale=1., name=None):
	constant=scale
	for i in shape[::-1]:
		try: constant=[constant]*i
		except: 
			print("exception in constant init! i is ",i, " the shape is ", shape)
			exit()
...
class kerasCudaConvnetPooling2DLayer(Layer):
    ...
    #def build(self, input_shape):
    #    super(kerasCudaConvnetPooling2DLayer, self).build(input_shape)  # Be sure to call this somewhere!
...
class kerasCudaConvnetConv2DLayer(Layer):
    def __init__(self, n_filters, filter_size, weights_std=0.01, init_bias_value=0.1, stride=1, activation='relu', partial_sum=None, pad=0, untie_biases=False,initW='normal',initB=constant ,initial_weights=None,W_regularizer=None,W_constraint=None, b_regularizer=None,b_constraint=None,**kwargs):
        """
	...
	self.initW = initializations.get(initW)
	self.initB = initializations.get(initB)
    ...
    def build(self, input_shape):
        if K.image_dim_ordering() != 'th': print "maybe wrong dim ordering in custom conv layer"

        self.filter_shape = (input_shape[0], self.filter_size, self.filter_size, self.n_filters)


        self.W = self.add_weight(self.filter_shape,
                                 initializer=functools.partial(self.initW, scale=self.weights_std),
        ...
        if self.untie_biases:
            self.b = self.add_weight((self.get_output_shape_for(input_shape)[:3]),
                                     initializer=functools.partial(self.initB, scale = self.init_bias_value),
	...
	self.built = True
        ...
        super(kerasCudaConvnetConv2DLayer, self).build(input_shape)  # Be sure to call this somewhere!
    ...
    def get_output_shape_for(self, input_shape):
	l,w,h,m_b=input_shape
        output_width = (w + 2 * self.pad - self.filter_size + self.stride) // self.stride
        output_height = (h + 2 * self.pad  - self.filter_size + self.stride) // self.stride        
