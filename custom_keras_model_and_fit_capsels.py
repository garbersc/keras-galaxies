...
class kaggle_winsol(Object):
    def __init__(self, BATCH_SIZE, NUM_INPUT_FEATURES, PART_SIZE, input_sizes, LEARNING_RATE_SCHEDULE, MOMENTUM, LOSS_PATH, METRICS_PATH, **kwargs):
    ...
    def loadWeights(self, modelname='model_norm', path):
    ...
    def evaluate(self, x, y, batch_size=self.BATCH_SIZE, modelname='model_norm_metrics'):
        ...
        for i in range(len(model_norm_metrics.metrics_names)):
            self.eval[modelname][model_norm_metrics.metrics_names[i]].append(
                evalHist[i])
