...
from keras.layers import Merge, Conv2D, MaxPooling2D
...
class kaggle_winsol(kaggle_base):
   ...
   Arguments:
    ...
    def init_models(self, optimizer=None):
        ...
        model.add(Conv2D(filters=32, kernel_size=6, use_bias=True, activation='relu',
                         kernel_initializer=initializers.TruncatedNormal(
                             stddev=0.01),
                         bias_initializer=initializers.Constant(value=0.1), name='conv_0'))
        model.add(MaxPooling2D(name='pool_0'))

        model.add(Conv2D(filters=64, kernel_size=5, use_bias=True, activation='relu',
                         kernel_initializer=initializers.TruncatedNormal(
                             stddev=0.01),
                         bias_initializer=initializers.Constant(value=0.1), name='conv_1'))
        model.add(MaxPooling2D(name='pool_1'))

        model.add(Conv2D(filters=128, kernel_size=3, use_bias=True, activation='relu',
                         kernel_initializer=initializers.TruncatedNormal(
                             stddev=0.1),
                         bias_initializer=initializers.Constant(value=0.1), name='conv_2'))
        model.add(Conv2D(filters=128, kernel_size=3, use_bias=True,
