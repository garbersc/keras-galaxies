...
def dense_weight_init_values( n_inputs , n_outputs, nb_feature = None , w_std = 0.001 , b_init_val = 0.01 , orth_style = False ):
	...
	weights = ( np.random.randn(*W_shape).astype(np.float32)*w_std , np.ones(n_outputs).astype(np.float32)*b_init_val )
	if orth_style:
		if len(n_inputs)!=1 or len(n_inputs)!=1: raise ValueError('For orthagonal matrixes in weight init the numer of input and output dimension must be one')
		if nb_feature: 
			weights = (weights[0] + np.repeat([np.eye(n_inputs[0],n_outputs[0],dtype=np.float32)-w_std/2], nb_feature , 0) , weights[1] ) 
		else: 
			weights = (weights[0] + np.eye(n_inputs[0],n_outputs[0],dtype=np.float32)-w_std/2 , weights[1] )
	return weights
